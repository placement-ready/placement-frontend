    ...
    return (
        <section className="max-w-7xl mx-auto px-6 py-10 grid grid-cols-1 md:grid-cols-2 gap-8 relative">
            ...
            <div className="absolute inset-0 pointer-events-none">
                ...
                <div className="absolute right-8 top-20 bottom-10 w-0.5 bg-gradient-to-b from-green-200 via-green-300 to-green-200 opacity-60 animate-line-glow" style={{ animationDelay: '0.5s' }}></div>
                ...
                <div className="absolute left-7 top-1/2 w-2 h-2 bg-emerald-500 rounded-full animate-pulse-dot" style={{ animationDelay: '1s' }}></div>
                <div className="absolute left-7 bottom-16 w-2 h-2 bg-emerald-400 rounded-full animate-pulse-dot" style={{ animationDelay: '2s' }}></div>
                
                <div className="absolute right-7 top-16 w-2 h-2 bg-green-400 rounded-full animate-pulse-dot" style={{ animationDelay: '0.5s' }}></div>
                <div className="absolute right-7 top-1/2 w-2 h-2 bg-green-500 rounded-full animate-pulse-dot" style={{ animationDelay: '1.5s' }}></div>
                <div className="absolute right-7 bottom-16 w-2 h-2 bg-green-400 rounded-full animate-pulse-dot" style={{ animationDelay: '2.5s' }}></div>
                    ...
                    <ul className="space-y-4">
                        {upcomingInterviews.map(({ id, company, position, dateTime }, index) => (
                            ...
                            >
                                ...
                                <p className="text-emerald-700 group-hover:text-emerald-800 group-hover:font-semibold transition-all duration-300">{position}</p>
                                <p className="mt-1 text-sm text-gray-600 group-hover:text-emerald-600 transition-colors duration-300">{dateTime}</p>
                            </li>
                        ))}
                    </ul>
                )}
                ...
                {latestSharedQuestions.length === 0 ? (
                    ...
                    <>
                            ...
                            {latestSharedQuestions.map(({ id, user, company, question, dateShared }, index) => (
                                ...
                                >
                                    ...
                                    <div className="flex justify-between items-center mb-2 relative z-10">
                                        <h3 className="font-semibold text-emerald-800 animate-pulse-gentle group-hover:text-emerald-900 group-hover:scale-105 transition-all duration-300">{company}</h3>
                                        <span className="text-sm text-gray-400 animate-fade-in group-hover:text-green-600 group-hover:font-medium transition-all duration-300" style={{ animationDelay: `${0.4 + index * 0.1}s` }}>{dateShared}</span>
                                    </div>
                                    <p className="text-gray-700 italic animate-typewriter relative z-10 group-hover:text-gray-800 group-hover:font-medium transition-all duration-300">"{question}"</p>
                                    <p className="mt-2 text-sm text-emerald-600 font-medium animate-bounce-subtle relative z-10 group-hover:text-emerald-700 group-hover:scale-105 transition-all duration-300">Shared by {user}</p>
                                </li>
                            ))}
                        </ul>
                        ...
                        <div className="flex justify-end mt-5">
                            <button
                                className="px-6 py-2 rounded-full bg-gradient-to-r from-emerald-500 to-green-500 text-white font-semibold shadow hover:shadow-lg hover:scale-105 transition-all duration-300"
                            >
                                Explore More
                            </button>
                        </div>
